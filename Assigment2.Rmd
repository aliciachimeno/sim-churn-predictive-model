...
vif(mod3)
```
It is not significant enough. We don't add it in our model. We can also see that *TotalCHarges* has a high VIF, so it has high multicollinearity.


```{r}
mod3.1 <- glm(Churn ~ (tenure + I(tenure^2) ) + MonthlyCharges , data=train, family=binomial)
mod3.1$deviance  
...
```{r}
mod3.2 <- glm(Churn ~ tenure + MonthlyCharges , data=train, family=binomial)
mod3.2$deviance  
anova( mod3.1, mod3.2,  test="Chisq")
summary(mod3.2) #aic 4473.5
vif(mod3.2)
```

Better without $I(tenure^2)$, we improved the multicollinearity.
...
##Inlfuential data
```{r}
infl <- influence.measures(mod3.2)
infl$infmat

cooks.distance(mod3.2)
sum(residuals(mod3.2,'deviance')^2)
sum(residuals(mod3.2,'pearson')^2)
```
...
```{r}
res_3.2 <- mod3.2$residuals

# Plotting a histogram of residuals
hist(res_3.2, main = "Histogram of Residuals", xlab = "Residuals")
...
cd_m3.2 <- cooks.distance(mod3.2)

# Plot Cook's distance
plot(cd_m3.2, pch = "18", main = "Cook's Distance Plot")
abline(h = 4/(length(mod3.2$coefficients) - 1), col = "red")

#Check homoscedasticity (constant variance of residuals) by plotting residuals against fitted values:

# Plotting residuals against fitted values

plot(fitted(mod3.2), res_3.2, pch = 16, main = "Residuals vs. Fitted Values", xlab = "Fitted Values", ylab = "Residuals")
```
...
anova( mod3.2, mod4,  test="Chisq") #significative
vif(mod4)
...
Having both interactions improves the model but VIF gets worse. The best mdoel is with SeniorCitizen and PaymentMethod interaction (mod21)
